<!-- Lane: C, State: ACTIVE — BuildWise MVP, 독립 서비스 (buildwise.ai 미배포). SSOT: ../docs/ARCHITECTURE.md §3 + ../docs/STATUS.md §2 -->
# CLAUDE.md
...
| `buildwise-backend/scripts/send_to_blender.py` | Blender MCP 스크립트 전송 (일반) |
| `buildwise-backend/scripts/send_large_to_blender.py` | Blender MCP 대용량 스크립트 전송 (파일 기반 exec) |
| `buildwise-backend/scripts/bmesh_sagrada_v4.py` | 사그라다 파밀리아 포토리얼 (최종, 42재질 135K verts) |
...
| 문서 | 용도 |
|------|------|
| `docs/PRD-blender-mcp-integration.md` | PRD: Blender MCP 3D 건물 생성 통합 (Phase 2) |
| `docs/DESIGN-blender-mcp-integration.md` | 설계서: Blender MCP 아키텍처 + 데이터 흐름 |
| `docs/blender-mcp-research.md` | Blender MCP + AI 3D 기술 조사 + 프로시저럴 건축 고급 기법 + Background-Mode 트랩 패턴 6종 |
| `docs/blender-mcp-scripts.md` | Blender MCP 프로시저럴 스크립트 카탈로그 (v1~v4 + 실행법) |
